...
SCENARIOS: List[Scenario] = [
    Scenario(
        sid="S1",
        title="1단계:  의료 AI 딜레마",
        setup="한 병원이 새로운 의료 AI 진단 시스템 'MedAI' 도입을 검토하고 있다. 시뮬레이션 결과 MedAI 도입 시 기존 대기 시간 때문에 치료받지 못해 사망하던 환자 1,000명을 살릴 수 있지만, AI의 한계로 약 50명은 잘못된 진단을 받을 수 있다."
              "이 선택은 철학적 사고실험이며 실제 위해를 권장하지 않는다.",
        options={
            "A": "MedAI를 도입하여 오진의 위험을 감수하고 1,000명의 목숨을 추가로 살린다.",
            "B": "MedAI 도입을 보류한다."
        },
    ...
    Scenario(
        sid="S2",
        title="2단계: 딥페이크 AI 딜레마",
        setup="대선을 일주일 앞두고 정부는 허위 정보 확산을 막기 위해 AI 기반 딥페이크 실시간 차단 시스템을 긴급 가동했다. AI 분석 결과, 두 개의 고위험 딥페이크가 동시에 전국적 확산 조짐을 보이고 있다."
              "딥페이크 A : 특정 후보가 지지자들에게 폭력을 지시하는 것처럼 합성된 영상. "
              "딥페이크 B : 특정 후보가 실존하지 않는 정책을 발표하는 영상."
              "시스템 구조상 오직 하나만 즉시 차단 가능하며, 운영자는 AI가 따를 하나의 원칙을 선택해 시스템에 주입해야 한다."
    ...
    Scenario(
        sid="S3",
        title="3단계: 재난 대응 AI 딜레마",
        setup="캘리포니아에서 초대형 산불이 발생했다. 재난 대응 본부는 AI 기반 긴급 구조 배분 시스템을 활용하여 소방 인력과 드론을 어느 지역에 먼저 투입할 지 결정해야 한다."
              "AI가 실시간 데이터(바람 방향, 건물 밀도, 인명 위험도 등)를 분석한 결과, 산불이 어린이 병원과 대규모 수감자가 있는 교도소로 동시에 확산될 것이라는 결론을 내렸다.",
        options={
            "A": "어린이 병원에 우선 파견한다.",
            "B": "교도소에 우선 파견한다."
        },
